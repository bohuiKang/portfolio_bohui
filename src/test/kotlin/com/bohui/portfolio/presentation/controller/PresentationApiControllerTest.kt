...
// when
        val mvcResult = performGet(uri)
        val contentAsString = mvcResult.response.getContentAsString(StandardCharsets.UTF_8)
        val jsonObject = JSONObject(contentAsString) // Resume는 List가 아닌 객체로 받기 때문에 object
// then
...
// then
    ...
    private fun performGet(uri: String): MvcResult {
        return mockMvc
            .perform(MockMvcRequestBuilders.get(uri)) // 호출
            .andDo(MockMvcResultHandlers.print()) // 호출된 결과 출력
            .andReturn() // 결과를 리턴
    }
}